// AFD recognizer (C89-compatible)
// Compile: gcc afd_recognizer_fixed.c -o afd_recognizer
...
int main(int argc, char **argv) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!in_trans) {
            ...
            unsigned char ch = (unsigned char)sym[0];
            trans[sidx][ch] = didx;
        }
    }
    fclose(f);

    int accept_set[MAX_STATES]; int i,j;
    for (i=0;i<MAX_STATES;i++) accept_set[i]=0;
